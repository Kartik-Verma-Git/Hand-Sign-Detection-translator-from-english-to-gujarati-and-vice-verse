...
class LanguageApp(MDApp):
    ...
    # Modify play_course_video to include updating progress
    # def play_course_video(self, video_path, user_id, course_id, video_id):
    #     # Existing video playback code
    #     cap = cv2.VideoCapture(video_path)
    #     if not cap.isOpened():
    #         self.show_dialog("Error: Could not open video file")
    #         return
    ...
    #     # Update progress after video finishes
    #     progress = update_progress(user_id, course_id, video_id)
    #     self.show_dialog(f"Video completed. Your progress is now {progress:.2f}%")
